...
namespace Editor {
	bool progressDialogueProgressFunctor(double progress, Common::Util::TimeMs elapsedTimeMs, void *userData) {
	...
	bool ProgressDialogue::progressFunctor(double progress, Common::Util::TimeMs elapsedTimeMs) {
		// Calculate some stuff
		char elapsedTimeStr[16];
		Common::Util::sprintTime(elapsedTimeStr, elapsedTimeMs);

		Common::Util::TimeMs remainingTimeMs=Common::Util::calculateTimeRemaining(progress, elapsedTimeMs);

		// Generate string and update dialogue text
		char str[1024]; // TODO: this better
		if (remainingTimeMs>=0) {
			char remainingTimeStr[16];
			Common::Util::sprintTime(remainingTimeStr, remainingTimeMs);
