...
namespace MapEditor {
	ClearDialogue::ClearDialogue(GtkWidget *parentWindow) {
		const char xmlFile[]="cleardialogue.glade";
		...
		if (gtk_builder_add_from_file(builder, xmlFile, &err)==0) {
			g_error_free(err);
			throw std::runtime_error("could not init clear dialogue glade file"); // TODO: use 'err->message' for more info
		}
	...
	bool ClearDialogue::run(Params *params) {
