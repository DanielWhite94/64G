...
namespace MapEditor {
	ContourLinesDialogue::ContourLinesDialogue(GtkWidget *parentWindow) {
		const char xmlFile[]="contourlinesdialogue.glade";
		...
		if (gtk_builder_add_from_file(builder, xmlFile, &err)==0) {
			g_error_free(err);
			throw std::runtime_error("could not init contour lines dialogue glade file"); // TODO: use 'err->message' for more info
		}
		...
		error|=(otherLineCountSpinButton=GTK_WIDGET(gtk_builder_get_object(builder, "otherLineCountSpinButton")))==NULL;
	...
	bool ContourLinesDialogue::run(Params *params) {
		// Update spin buttons based on given values
		gtk_spin_button_set_value(GTK_SPIN_BUTTON(otherLineCountSpinButton), params->lineCount);
		...
		params->lineCount=gtk_spin_button_get_value(GTK_SPIN_BUTTON(otherLineCountSpinButton));
