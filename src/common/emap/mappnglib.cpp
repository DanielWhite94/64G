...
namespace Common {
				...
				for(imageY=imageY0; imageY<imageY1; ++imageY) {
					for(imageX=imageX0; imageX<imageX1; ++imageX) {
						...
						if (regionExists) {
							...
							tile=map->getTileAtCoordVec(vec, Common::EMap::Map::GetTileFlag::None);
							if (tile==NULL) {
	...
	void MapPngLib::getColourForTileHeightContour(class Map *map, int mapTileX, int mapTileY, const MapTile *tile, uint8_t *r, uint8_t *g, uint8_t *b, uint8_t *a) {
		...
		*r=*g=*b=*a=0;
		...
		neighbourTile=map->getTileAtOffset(map->decTileOffsetX(mapTileX), mapTileY, Common::EMap::Map::GetTileFlag::None);
		if (neighbourTile!=NULL && neighbourTile->getHeight()<=contourHeight) {
			*a=255;
			return;
		}
		neighbourTile=map->getTileAtOffset(map->incTileOffsetX(mapTileX), mapTileY, Common::EMap::Map::GetTileFlag::None);
		if (neighbourTile!=NULL && neighbourTile->getHeight()<=contourHeight) {
			*a=255;
			return;
		}
		neighbourTile=map->getTileAtOffset(mapTileX, map->decTileOffsetX(mapTileY), Common::EMap::Map::GetTileFlag::None);
		if (neighbourTile!=NULL && neighbourTile->getHeight()<=contourHeight) {
			*a=255;
			return;
		}
		neighbourTile=map->getTileAtOffset(mapTileX, map->incTileOffsetX(mapTileY), Common::EMap::Map::GetTileFlag::None);
		if (neighbourTile!=NULL && neighbourTile->getHeight()<=contourHeight) {
			*a=255;
			return;
		}
