				...
				for(unsigned rX=0; rX<rXEnd; ++rX) {
					...
					for(startY=tileY0; startY<tileY1; ++startY) {
						for(startX=tileX0; startX<tileX1; ++startX) {
							...
							MapTile *tile=map->getTileAtOffset(startX, startY, EMap::Map::GetTileFlag::None);
							...
							while(!segments.empty()) {
								...
								while(1) {
									...
									MapTile *loopTile=map->getTileAtOffset(loopX, segment.y, EMap::Map::GetTileFlag::Dirty);
									if (loopTile==NULL) {
									...
									if (!aboveIsBoundary) {
										MapTile *aboveTile=map->getTileAtOffset(loopX, aboveSegment.y, EMap::Map::GetTileFlag::None);
									...
									if (!belowIsBoundary) {
										MapTile *belowTile=map->getTileAtOffset(loopX, belowSegment.y, EMap::Map::GetTileFlag::None);
