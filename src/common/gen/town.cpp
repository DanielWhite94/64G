		...
		bool addTown(class Map *map, unsigned x0, unsigned y0, unsigned x1, unsigned y1, const AddTownParameters *params, const AddHouseParameters *houseParams, int townPop) {
			...
			while(!pq.empty()) {
				...
				for(a=road.y0; a<road.trueY1; ++a)
					for(b=road.x0; b<road.trueX1; ++b){
						MapTile *newTile=map->getTileAtCoordVec(CoordVec(b*Physics::CoordsPerTile, a*Physics::CoordsPerTile), EMap::Map::GetTileFlag::CreateDirty);
						if (newTile!=NULL)
			...
			for(auto houseData : houses) {
				...
				if (houseParams->flags & AddHouseFlags::ShowDoor) {
					...
					// Add sign.
					MapTile *newTile=map->getTileAtCoordVec(CoordVec(signX*Physics::CoordsPerTile, (houseData.y+houseData.mapH-2)*Physics::CoordsPerTile), EMap::Map::GetTileFlag::CreateDirty);
					if (newTile!=NULL) {
