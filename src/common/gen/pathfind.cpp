...
namespace Common {
	namespace Gen {
		...
		float pathFindDistanceFunctorDistanceWeight(class Map *map, unsigned x1, unsigned y1, unsigned x2, unsigned y2, void *userData) {
			...
			// Grab both tiles
			MapTile *tile1=map->getTileAtOffset(x1, y1, Common::EMap::Map::GetTileFlag::None);
			MapTile *tile2=map->getTileAtOffset(x2, y2, Common::EMap::Map::GetTileFlag::None);
		...
		float pathFindSearchGoalDistanceHeuristicDistanceWeight(class Map *map, unsigned x1, unsigned y1, unsigned x2, unsigned y2, void *userData) {
			...
			MapTile *t1=map->getTileAtOffset(x1, y1, Common::EMap::Map::GetTileFlag::None);
			MapTile *t2=map->getTileAtOffset(x2, y2, Common::EMap::Map::GetTileFlag::None);
			if (t1!=NULL && t2!=NULL)
