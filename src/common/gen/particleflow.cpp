...
namespace Common {
	namespace Gen {
		...
		void ParticleFlow::dropParticles(unsigned x0, unsigned y0, unsigned x1, unsigned y1, double coverage, unsigned threadCount, Util::ProgressFunctor *progressFunctor, void *progressUserData) {
			...
			for(auto const& regionPos: regionList) {
				...
				for(unsigned i=0; i<trials; ++i) {
					...
					// Grab tile.
					const MapTile *tile=map->getTileAtOffset((int)floor(tileX), (int)floor(tileY), EMap::Map::GetTileFlag::None);
					if (tile==NULL)
						continue;
		...
		void ParticleFlow::dropParticle(double xp, double yp) {
			...
			for(int pathLen=0; pathLen<maxPathLen; ++pathLen) {
				...
				if (incMoisture) {
					...
					// Lookup tile
					MapTile *tempTile=map->getTileAtOffset(tempX, tempY, EMap::Map::GetTileFlag::None);
					if (tempTile!=NULL)
		...
		double ParticleFlow::hMap(int x, int y, double unknownValue) {
			...
			// Grab tile.
			const MapTile *tile=map->getTileAtOffset(x, y, EMap::Map::GetTileFlag::None);
			if (tile==NULL)
				return unknownValue;
		...
		void ParticleFlow::depositAt(int x, int y, double w, double ds) {
			...
			// Grab tile.
			MapTile *tile=map->getTileAtOffset(x, y, EMap::Map::GetTileFlag::Dirty);
			if (tile==NULL)
				return;
