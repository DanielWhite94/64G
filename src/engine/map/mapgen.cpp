...
namespace Engine {
	namespace Map {
		...
		class Map *MapGen::generate(void) {
			// Choose parameters.
			const unsigned mapWidth=1024;
			const unsigned mapHeight=1024;
			const double cellWidth=1.0;
			const double cellHeight=1.0;
			const double heightResolution=200.0;

			double *heightArray=(double *)malloc(sizeof(double)*mapHeight*mapWidth);
			assert(heightArray!=NULL); // TODO: better
			double *heightArrayPtr;

			unsigned yProgressDelta=mapHeight/16;
			...
			float freqFactorX=(cellWidth/mapWidth)*1024.0;
			float freqFactorY=(cellHeight/mapHeight)*1024.0;
			heightArrayPtr=heightArray;
			for(y=0;y<mapHeight;++y) {
				for(x=0;x<mapWidth;++x,++heightArrayPtr)
				...
				if (y%yProgressDelta==yProgressDelta-1)
					printf("MapGen: base generation %.1f%%.\n", ((y+1)*100.0)/mapHeight); // TODO: this better
			}
