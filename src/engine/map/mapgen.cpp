...
namespace Engine {
	namespace Map {
		...
		void MapGen::EdgeDetect::trace(SampleFunctor *sampleFunctor, void *sampleUserData, EdgeFunctor *edgeFunctor, void *edgeUserData, ProgressFunctor *progressFunctor, void *progressUserData) {
			...
			Gen::modifyTiles(map, 0, 0, mapWidth, mapHeight, 1, &Gen::modifyTilesFunctorBitsetIntersection, (void *)(uintptr_t)~scratchBitMask, (progressFunctor!=NULL ? &mapGenEdgeDetectTraceClearScratchBitsModifyTilesProgressFunctor : NULL), &modifyTileFunctorData);
		...
		double MapGen::narySearch(class Map *map, unsigned x, unsigned y, unsigned width, unsigned height, unsigned threadCount, int n, double threshold, double epsilon, double sampleMin, double sampleMax, NArySearchGetFunctor *getFunctor, void *getUserData) {
			...
			for(int iter=0; iter<iterMax; ++iter) {
				...
				Gen::modifyTiles(data.map, x, y, width, height, threadCount, &mapGenNArySearchModifyTilesFunctor, &data, &Gen::modifyTilesProgressString, (void *)progressString);
				printf("\n");
