...
namespace Engine {
	namespace Map {
		...
		public:
			...
			static void generateTileMap(const class Map *map, unsigned zoom, unsigned x, unsigned y, unsigned depth); // If the image already eists, nothing is done, otherwise if all children exist the image is stitched together from them. Otherwise if depth==0 will then generate manually by calling mappng, if depth>0 then will recurse with depth-1 to generate children before stitching together as normal.
